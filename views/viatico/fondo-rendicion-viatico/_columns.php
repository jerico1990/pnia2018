<?php
use yii\helpers\Url;
use app\controllers\Viatico\FondoRendicionViaticoController;
...
<script type="text/javascript">
    ...
    function cargarRendicionGenerica(fondo_rendicion_viatico_id){
        $.ajax({
            url: "<?= Url::base() ?>/Viatico/gestion-rendicion-viatico/enviar-codigo-rendicion",
            method: 'POST',
            data: {
                fondo_rendicion_viatico_id : fondo_rendicion_viatico_id,
                //codigo_requerimiento : codigo_requerimiento,
            },
            success:function(text){
                //alert(codigo_interno_contrato_visible);
                var tablas_anexas = document.getElementById('detalle-rendicion-viatico');
                tablas_anexas.style.display = 'block';
                $.pjax.reload({
                    container:'#crud-datatable-rendicion-viatico-pjax',
                });
    ...
    [
        ...
        'content' => function($model,$key,$index,$column){
            return Html::button(
                'Detalle',
                array(
                    'id'=>'buttonDetalleRendicion'.$model->fondo_rendicion_viatico_id,
                    'onclick'=>'cargarRendicionGenerica("'.$model->fondo_rendicion_viatico_id.'")',
