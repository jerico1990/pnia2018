...
<div class="requerimiento-index">
    <div id="ajaxCrudDatatable">
        <?=GridView::widget([
            ...
            'pjax'=>true,
            'columns' => require(__DIR__.'/_columns_autorizacion_jefe_requerimiento.php'),
            'toolbar'=> [
                ['content'=>
            ...
            'panel' => [
                'type' => 'primary',
                'heading' => '<i class="glyphicon glyphicon-list"></i> Requerimientos listing - jefe',
...
$('body').on('click', '.btn-aprobar-jefe-requerimiento', function (e) {
    e.preventDefault();
    var requerimiento_id = $(this).attr('data-id');
    var msg = confirm("¿Esta seguro de procesar la aprobación del requerimiento?");
    if (msg == true) {
      $.ajax({
          url:'<?= \Yii::$app->request->BaseUrl ?>/Adquisicion/requerimiento/aprobar-jefe-requerimiento',
          async:false,
...
$('body').on('click', '.btn-desaprobar-jefe-requerimiento', function (e) {
    e.preventDefault();
    var requerimiento_id = $(this).attr('data-id');
    var msg = confirm("¿Esta seguro de procesar la desaprobación del requerimiento?");
    if (msg == true) {
      $.ajax({
          url:'<?= \Yii::$app->request->BaseUrl ?>/Adquisicion/requerimiento/desaprobar-jefe-requerimiento',
          async:false,
          data:{requerimiento_id:requerimiento_id},
          method: 'POST',
          beforeSend:function()
